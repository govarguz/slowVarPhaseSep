          ...
          for (std::vector<Cell*>::iterator it = realCells.begin(),
            ...
            for (size_t p = 0; p < cell.particles.size(); ++p) {
              ...
              if (pos[coord] - cellGrid.getMyLeft(coord) < -ROUND_ERROR_PREC) {
                ...
                particlesHangoverCounters[coord]++;
              ...
              // check whether the particle is now "right" of the local domain
                ...
                --p;
                particlesHangoverCounters[coord+3]++;
              }
...
//ADDH
  void DomainDecomposition::resetParticlesHangoverCounters() {
    for(int i = 0; i < 6; i++)
      particlesHangoverCounters[i] = 0;
  }

  //using namespace boost::python::object;

  static boost::python::object wrapGetCounters(class DomainDecomposition* obj) {
      int pchs[6];
      obj->loadCounters(pchs);
      return boost::python::make_tuple(pchs[0],pchs[1],pchs[2],pchs[3],pchs[4],pchs[5]);
  }

  void DomainDecomposition::loadCounters(int pcs[6]) {
    pcs[0] = particlesHangoverCounters[0];
    pcs[1] = particlesHangoverCounters[1];
    pcs[2] = particlesHangoverCounters[2];
    pcs[3] = particlesHangoverCounters[3];
    pcs[4] = particlesHangoverCounters[4];
    pcs[5] = particlesHangoverCounters[5];
  }
  ...
  void DomainDecomposition::resetParticlesCommCellsCounters() {
    for(int i = 0; i < 6; i++)
      particlesCommCellsCounters[i] = 0;
  }

  static boost::python::object wrapGetCommCellsTotalParticles(class DomainDecomposition* obj) {
      int pcchs[6];
      obj->loadCommCellsCounters(pcchs);
      return boost::python::make_tuple(pcchs[0],pcchs[1],pcchs[2],pcchs[3],pcchs[4],pcchs[5]);
  }

  void DomainDecomposition::loadCommCellsCounters(int pccs[6]) {
    pccs[0] = particlesCommCellsCounters[0];
    pccs[1] = particlesCommCellsCounters[1];
    pccs[2] = particlesCommCellsCounters[2];
    pccs[3] = particlesCommCellsCounters[3];
    pccs[4] = particlesCommCellsCounters[4];
    pccs[5] = particlesCommCellsCounters[5];
  }
      ...
      */
      ...
      for (int lr = 0; lr < 2; ++lr) {
        ...
        if (nodeGrid.getGridSize(coord) == 1) {
	      ...
	      particlesCommCellsCounters[coord+3]=sum;	
	      ...
	      sum = 0;
  	      for (int i = 0; i<recvSizes.size(); ++i){	
              ...
              particlesCommCellsCounters[coord]=sum;			
              }
            else {
  ...
  void DomainDecomposition::registerPython() {
    ...
    .def("getCounters", &wrapGetCounters)
    .def("getCommCellsTotalParticles", &wrapGetCommCellsTotalParticles)
    .def("resetParticlesHangoverCounters", &DomainDecomposition::resetParticlesHangoverCounters)
    .def("resetParticlesCommCellsCounters", &DomainDecomposition::resetParticlesCommCellsCounters)
