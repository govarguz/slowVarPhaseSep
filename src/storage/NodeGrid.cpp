  ...
  namespace storage {
      ...
      : Grid(grid)
    {
      ...
      // ###################################################################################
        ...
        //std::cout<<"DD.cpp---H--- How size works : " << maxDomainSizeInCells[0] <<"\n";
        maxDomainSizeInCells[1]=neiListy[neiListy.size()-1];
        maxDomainSizeInCells[2]=neiListz[neiListz.size()-1];

       // ################################   H   ############################################
       // Defining localDomainBoxes with asteroids
       // ###################################################################################
       // Int3D localNodePos;
...
//std::cout<<"DD.cpp---H--- How size works : " <<kCellGridSize <<"\n";
      for (int i = 0; i < 3; ++i) {
          //std::cout<<"DD.cpp---H--- the pos is: " <<pos[i] <<"\n";
          cpos[i] = static_cast< int >(pos[i]/kCellGridSize);
          //std::cout<<"DD.cpp---H--- the cpos is: " <<cpos[i] <<"\n";
        if (cpos[i] <= 0) {
          cpos[i] = 0;
        }
        ...
        //std::cout<<"DD.cpp---H--- the cpos is: " <<cpos <<"\n";
      }
