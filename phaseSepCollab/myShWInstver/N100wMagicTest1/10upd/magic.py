...
def runMagic2(pidf,  xposf, yposf, zposf, xvelf, yvelf, zvelf, box_x, box_y, box_z,bending,num_chains,monomers_per_chain,nodeGrid,rc,skin,cellGrid,ktheta,dt,temperature1,temperature2,pt1,pt2,pt3,pt4,neiListxIn=[], neiListyIn=[], neiListzIn=[]): #
    ...
    if len(neiListxIn)>1:
        ...
        coRperPlane=1 #2.5  # Optimization to be implemented soon
        drive=0 # This Flag makes the KPIs checks only for Part+Ints or if drive=1 all KPIs
        neiListx, neiListy, neiListz =criteriaInst(pt1,pt2,pt3,pt4,neiListxIn,neiListyIn,neiListzIn,coRperPlane,drive,None,threshNParts=2.,threshNInts=2.,threshNHangs=68.,threshNComms=10)
...
def criteriaInst(pt1,pt2,pt3,pt4,neiListxIn,neiListyIn,neiListzIn,coRperPlane=1.00,drive=0,filename=None,threshNParts=0,threshNInts=0,threshNHangs=0,threshNComms=0):
    #coRperPlane=2.00
    ...
    if filename==None:
        ...
        if pt1C>coRperPlane*1.:
            ...
            #else:
                if any([pt3CxL,pt3CxR,pt3CyL,pt3CyR,pt3CzL,pt3CzR]):
                   print "WARNING: not implemented yet...(add ifs and control infra...) threshNHangs"
                   ddFlag=1
             #   else:
                   if any([pt4CxL,pt4CxR,pt4CyL,pt4CyR,pt4CzL,pt4CzR]):
