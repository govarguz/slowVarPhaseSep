# -*- coding: utf-8 -*-
"""
Created on Mon Jun 13 12:22:16 2016
...
system=[[[[[]]]]]*5
system[0]         = espressopp.System()
system[0].rng     = espressopp.esutil.RNG()
system[0].bc      = espressopp.bc.OrthorhombicBC(system[0].rng, box)
system[0].skin    = skin
...
system[0].storage = espressopp.storage.DomainDecomposition(system[0], nodeGrid, cellGrid,neiListx,neiListy,neiListz)
  
integrator     = espressopp.integrator.VelocityVerlet(system[0])  
integrator.dt  = dt

#LANGEVIN DOUBLE THERMOSTAT
thermostat             	= espressopp.integrator.LangevinThermostat2TId(system[0])
...
bondlist = espressopp.FixedPairList(system[0].storage)
if bending:
	anglelist = espressopp.FixedTripleList(system[0].storage)
...
for i in range(num_chains):
	...
	system[0].storage.addParticles(chain, *props)
	system[0].storage.decompose()
...
#LJ nonbonded
vl 						 = espressopp.VerletList(system[0], cutoff=rc)
...
system[0].addInteraction(interLJ00)

# FENE bonds
potFENE   = espressopp.interaction.FENE(K=30.0, r0=0.0, rMax=1.5)
interFENE = espressopp.interaction.FixedPairListFENE(system[0], bondlist, potFENE)
system[0].addInteraction(interFENE)

#bending interation - only if bending switched on
# Cosine with FixedTriple list
if bending:
	potCosine = espressopp.interaction.Cosine(K=ktheta, theta0=3.1415926)
	interCosine = espressopp.interaction.FixedTripleListCosine(system[0], anglelist, potCosine)
	system[0].addInteraction(interCosine)
...
'''
system[0].storage.resetParticlesHangoverCounters()
system[0].storage.resetParticlesCommCellsCounters()

flagI=True
espressopp.tools.myfastwritexyz(outconfpath+'prc_'+fid+'t'+str(int(t))+'.dat',system[0], velocities=True, unfolded = True,mpc = monomers_per_chain)# mywrite writes the type as particleId/monomers_per_chain %2
...
for j in range(nsteps): 		#run nsteps x samp time
    espressopp.tools.analyse.info(system[sysCounter],integrator)
    integrator.run(100)#1000000#tsamp*tausteps) 		#run for samp time * \tau
    if mdSteps>199:#3.9999e6: #Tunned Param for J Smrek
        if cheInK==inK or flagI==True:        
            l=len(system[sysCounter].getInteraction(0).getVerletList().getAllPairs())
            p=[]
            for k2 in range(l):
                p.append(len(system[sysCounter].getInteraction(0).getVerletList().getAllPairs()[k2]))
            pt1.append(p)
            #print "pt1 IS:",pt2    
            pt2.append(espressopp.analysis.NRealPart(system[sysCounter]).compute())
            print "pt2 IS:",pt2
            pt3.append(system[sysCounter].storage.getCounters())
            pt4.append(system[sysCounter].storage.getCommCellsTotalParticles())
            ...
            system[sysCounter].storage.resetParticlesHangoverCounters()
            system[sysCounter].storage.resetParticlesCommCellsCounters()
            cheInK=cheInK+1
        ...
        #print "Passed Here"
        dd=checkDDdyn(pt1,pt2,threshNParts=2.,threshNInts=20.,coRperPlane=1.) #threshNParts=26.,threshNInts=270.,coRperPlane=1.5)
        if dd==True:
            pidf2, xposf2, yposf2, zposf2, xvelf2, yvelf2, zvelf2, box_x, box_y, box_z=myGatherConfOTF(system[sysCounter],velocities=True,unfolded=True,append=False,mpc=monomers_per_chain)
            espressopp.FixedPairList.remove(bondlist)
            espressopp.FixedTripleList.remove(anglelist)  
    	    #del(system)
            sysCounter=sysCounter+1
            #espressopp.pmi.sync()
            print "Las Vars :  "
            system[sysCounter],integrator=runMagic2(pidf2, xposf2, yposf2, zposf2, xvelf2, yvelf2, zvelf2, box_x, box_y, box_z,bending,num_chains,monomers_per_chain,nodeGrid,rc,skin,cellGrid,ktheta,dt,temperature1,temperature2,pt1,pt2,pt3,pt4,neiListx, neiListy, neiListz)	    			
            #espressopp.pmi.sync()
    ...
    t = integrator.step*integrator.dt + tin
    mdSteps=mdSteps+tsamp*tausteps
    espressopp.tools.myfastwritexyz(outconfpath+'prc_'+fid+'t'+str(int(t))+'.dat',system[sysCounter], velocities=True, unfolded = True, mpc = monomers_per_chain)
...
fintime = time.time()
espressopp.tools.analyse.final_info(system[sysCounter], integrator, vl, intime, fintime)
