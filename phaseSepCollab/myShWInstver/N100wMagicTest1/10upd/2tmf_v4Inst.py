...
for j in range(nsteps): 		#run nsteps x samp time
    ...
    integrator.run(1000)#1000000#tsamp*tausteps) 		#run for samp time * \tau
    if mdSteps>1999:#3.9999e6: #Tunned Param for J Smrek
        ...
        if dd==True:
            ...
            #espressopp.pmi.sync()
            system,integrator=runMagic2(pidf2, xposf2, yposf2, zposf2, xvelf2, yvelf2, zvelf2, box_x, box_y, box_z,bending,num_chains,monomers_per_chain,nodeGrid,rc,skin,cellGrid,ktheta,dt,temperature1,temperature2,pt1,pt2,pt3,pt4,neiListx, neiListy, neiListz)	    			
            espressopp.pmi.sync()
