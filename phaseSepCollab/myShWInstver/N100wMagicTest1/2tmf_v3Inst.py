...
import csv
from magic import runMagic,myGatherConfOTF,runMagic2
...
for j in range(nsteps): 		#run nsteps x samp time
    ...
    integrator.run(10000)#1000000#tsamp*tausteps) 		#run for samp time * \tau
    if mdSteps>9999999999:#3.9999e6: #Tunned Param for J Smrek
        l=len(system.getInteraction(0).getVerletList().getAllPairs()) 
        p=[]
        for k2 in range(l):
            p.append(len(system.getInteraction(0).getVerletList().getAllPairs()[k2]))
        p2=p
	print "my P type is:",type(p2)
	pt1.append(p)
	p3=espressopp.analysis.NRealPart(system).compute()
	print "my NPart type is:",type(p3)
	print "p3 first elem",p3[0]
        pt2.append(espressopp.analysis.NRealPart(system).compute())
	p4=system.storage.getCounters()
	print "my type Hangovers is:",type(p4)
	print "the parts",p4
	print "p4 first elem",p4[7][2]
        pt3.append(system.storage.getCounters())
	print "my CommCells counters are:",type(system.storage.getCommCellsTotalParticles())
	    ...
	    del(system)
            system=runMagic2(pidf2, xposf2, yposf2, zposf2, xvelf2, yvelf2, zvelf2, box_x, box_y, box_z,bending,num_chains,monomers_per_chain,nodeGrid,rc,skin,cellGrid,ktheta,dt,temperature1,temperature2)
